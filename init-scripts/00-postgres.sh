...
DATABASE_DIR="${DATABASE_DIR_POSTGRES:-/data/db/sqlite}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
WWW_ROOT_DIR="/usr/share/httpd/default"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Default predefined variables
DATA_DIR="/data/postgres"   # set data directory
...
RUN_DIR="/run/postgres"       # set scripts pid dir
...
SERVICE_GROUP="postgres" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
[ -f "$CONF_DIR/env/postgres.sh" ] && . "$CONF_DIR/env/postgres.sh"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  if __is_dir_empty "$DATABASE_DIR" [ ! -f "$DATABASE_DIR/PG_VERSION" ] || [ ! -s "$DATABASE_DIR/PG_VERSION" ]; then
    cd "/tmp" || exit
    mkdir -p "$DATABASE_DIR" && chmod -f 0750 "$DATABASE_DIR"
    chown -Rf postgres:postgres /config/secure/auth/*/postgres*
    rm -Rf "${DATABASE_DIR:?DATABASE_DIR is not set}"/*
    sudo -u $user initdb --username="$user" -A md5 --pwfile="${ROOT_FILE_PREFIX}/${SERVICE_NAME}_pass" -D "$DATABASE_DIR" |& tee -a "$LOG_DIR/init.txt" &>/dev/null
  fi
...
__post_execute() {
  ...
  (
    sleep 20
    DATABASE_NAME="$DATABASE_CREATE" DB_USER_NAME="$user_name" DB_USER_PASS="$user_pass" ADMIN_USER="$root_user_name" ADMIN_PASS="$root_user_pass" db_init_ppgsql
  ) |& tee -p -a "$LOG_DIR/init.txt" &>/dev/null &
  return $exitCode
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$POSTGRES_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$POSTGRES_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$POSTGRES_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$POSTGRES_USER_PASS_WORD}"             #
